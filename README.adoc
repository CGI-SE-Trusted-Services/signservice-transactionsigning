...
[source]
----
implementation 'se.signatureservice.support:signservice-transactionsigning:2509.1'
----
...
==== Multiple TransactionSigner instances [[signer]]

It is quite possible to create several TransactionSigner instances. However, when using the
above recipe, the separate instances will share configuration, making it less useful. This is the legacy behavior.

In order to have several TransactionSigner instances with separate configurations, in the same java process,
the instances have to be created using the 'buildIsolatedInstance' method, instead of the regular 'build' method.
...
==== Multiple TransactionValidator instances [[validator]]

Just like with TransactionSigner, separate instances of TransactionValidator will share configuration, unless
the 'buildIsolatedInstance' method is used, instead of the regular 'build' method.
