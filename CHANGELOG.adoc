...
* Use the signing date from the remote-sign server, to calculate reference id:s
based on the same data as the remote-sign server.

== Version 2602.1

=== Changes

* Upgraded Gradle to 9.3.1.
* Upgraded Spock to 2.4.
* Edited bundled validation policy, removing the iIDAS tag.
* Added a few more eu.europa.ec.joinup.sd-dss runtime dependencies.
...
* It is now possible to create separately configured TransactionSigner and TransactionValidator instances, see
<<README.adoc#signer,here>>, and <<README.adoc#validator,here>>.

=== Changes

* Upgraded Gradle to 8.14.3.
* Added runtime dependency eu.europa.ec.joinup.sd-dss:dss-cms-object to project.
